...
const totalRevenue = computed(() => tableData.value.reduce((sum, r) => sum + r.revenue, 0));
const totalCommissions = computed(() => tableData.value.reduce((sum, r) => sum + r.commission, 0));
const totalNetPayout = computed(() => tableData.value.reduce((sum, r) => sum + r.net, 0));
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all duration-300">
          ...
          <div class="relative z-10 flex justify-between items-start">
            <div>
              ...
              <h3 class="text-3xl font-black text-slate-800">฿{{ formatPrice(totalRevenue) }}</h3>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all duration-300">
          ...
          <div class="relative z-10 flex justify-between items-start">
            <div>
              ...
              <h3 class="text-3xl font-black text-rose-600">฿{{ formatPrice(totalCommissions) }}</h3>
              <p class="text-xs font-bold text-rose-400 mt-4 flex items-center gap-1">
                ...
                Avg Rate: {{ ((totalCommissions / (totalRevenue || 1)) * 100).toFixed(1) }}%
              </p>
            </div>
          ...
          <table class="table w-full">
            ...
            <tfoot class="bg-slate-50/50">
              <tr>
                ...
                <th class="text-right">฿{{ formatPrice(totalRevenue) }}</th>
                <th></th>
                <th class="text-right">฿{{ formatPrice(totalCommissions) }}</th>
                <th class="text-right">฿{{ formatPrice(totalNetPayout) }}</th>
              </tr>
            </tfoot>
          </table>
        </div>
