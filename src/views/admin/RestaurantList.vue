              ...
              <tr v-for="restaurant in restaurantStore.list" :key="restaurant.id" class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <div class="flex items-center gap-4">
                    <div class="avatar">
                      <div class="mask mask-squircle w-12 h-12 bg-slate-100">
                        <img v-if="restaurant.ImageUrl" :src="restaurant.ImageUrl" :alt="restaurant.Restaurant" class="object-cover" />
                        <div v-else class="flex items-center justify-center h-full text-[10px] text-slate-400">No Image
                        </div>
                      </div>
                    </div>
                    <div>
                      <div class="font-bold text-indigo-600">{{ restaurant.Restaurant }}</div>
                    </div>
                  </div>
                </td>
                ...
                <td class="text-center">
                  <div class="flex justify-center items-center gap-2">
                    <RouterLink :to="{ name: 'Admin Restaurant Detail', params: { name: restaurant.Restaurant } }" class="btn btn-sm btn-ghost text-indigo-500 hover:bg-indigo-50">
