...
const checkSave = () => formStore.save(route.params.id || null, router, route);
                        ...
                        <div class="flex flex-col items-center gap-5 w-full max-w-xs">
                            ...
                            <div class="flex flex-col gap-4 w-full">
                                <div class="w-full">
                                    ...
                                    <div v-if="isEdit" class="text-[10px] text-slate-400 mt-1 text-center">รองรับไฟล์ .jpg, .png ขนาดไม่เกิน 5MB</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="p-8 lg:col-span-2 space-y-8">
                        <div class="space-y-8">
                            <div>
                                ...
                                <div v-if="isEdit" class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 mb-4">
                                ...
                                <h3 class="font-bold text-slate-700 mb-4 border-b border-slate-100 pb-2">{{ isEdit ? 'ข้อมูลทั่วไป' : 'ข้อมูล' }}</h3>
                                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                                    ...
                                    <div class="form-control md:col-span-2">
                                        ...
                                        <div class="relative">
                                            <input type="number" class="input input-bordered w-full pr-10 text-right focus:input-primary bg-slate-50 border-slate-200" v-model="MenuData.Price" @keypress="onlyNumber" />
                                    ...
                                    <div class="form-control md:col-span-2">
                                        ...
                                        <div class="relative">
                                            <input type="number" class="input input-bordered w-full pr-10 text-right focus:input-primary bg-slate-50 border-slate-200 text-slate-700" v-model="MenuData.PromoPrice" @keypress="onlyNumber" />
                                            <span class="absolute right-4 top-3 text-slate-400 text-sm">฿</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-control md:col-span-2">
                                        <label class="label"><span class="label-text font-medium text-slate-600">ร้านอาหาร</span></label>
                                        <select class="select select-bordered w-full focus:select-primary bg-slate-50 border-slate-200 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed" v-model="MenuData.Restaurant" :disabled="true">
                                            <option disabled value="">เลือกร้านอาหาร</option>
                                            <option v-for="res in Restaurant.list" :key="res.id" :value="res.RestaurantName">{{ res.RestaurantName }}</option>
                                        </select>
                                    </div>
                                    <div class="form-control md:col-span-2">
                                        <label class="label"><span class="label-text font-medium text-slate-600">หมวดหมู่อาหาร</span></label>
                                        <select class="select select-bordered w-full focus:select-primary bg-slate-50 border-slate-200" v-model="MenuData.Category">
                                            <option disabled value="">เลือกหมวดหมู่</option>
                                            <option v-for="cat in categoryStore.list" :key="cat.id" :value="cat.Category">{{ cat.Category }}</option>
                                        </select>
                                    </div>
                                    <div class="form-control md:col-span-2">
                                        <label class="label"><span class="label-text font-medium text-slate-600">สถานะการขาย</span></label>
                        ...
                        <div class="mt-12">
                            ...
                            <div class="space-y-6">
                                    ...
                                    <div v-for="(group, gIndex) in MenuData.OptionGroups" :key="'group-' + gIndex" class="relative pb-6 border-b border-slate-100 last:border-0 last:pb-0 group">
                                        ...
                                        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-4 pr-10 items-end">
                                            <div class="form-control">
                                                <label class="label"><span class="label-text font-medium text-slate-600">ชื่อหมวดหมู่ตัวเลือก</span></label>
                                                <input type="text" class="input input-bordered w-full focus:input-primary bg-slate-50 border-slate-200" v-model="group.name" />
                                            </div>
                                            <div class="form-control">
                                                <label class="label"><span class="label-text font-medium text-slate-600">จำนวนที่เลือกได้</span></label>
                                                <div class="relative">
                                                    <input type="number" min="1" class="input input-bordered w-full pr-16 focus:input-primary bg-slate-50 border-slate-200" v-model="group.maxChoices" @keypress="onlyNumber" />
                                                    <span class="absolute right-4 top-3 text-slate-400 text-sm font-medium">รายการ</span>
                                                </div>
                                            </div>
                                            <div class="form-control">
                                                <label class="label"><span class="label-text font-medium text-slate-600">สถานะการเลือก</span></label>
                                                <div class="relative">
                                                    <select class="select select-bordered w-full focus:select-primary bg-slate-50 border-slate-200" v-model="group.isRequired">
                                        ...
                                        <div class="pl-2 md:pl-4 border-l-2 border-slate-100">
                                            <div class="flex items-center justify-between mb-2">
                                                <label class="label px-0"><span class="label-text font-medium text-slate-600">ตัวเลือก</span></label>
                                            </div>
                                            <div class="space-y-3 mb-4">
                                                <div v-for="(choice, cIndex) in group.choices" :key="'choice-' + gIndex + '-' + cIndex" class="flex items-start gap-4">
                                                    <div class="form-control flex-1">
                                                        <input type="text" class="input input-sm input-bordered w-full focus:input-primary bg-slate-50 border-slate-200 h-10" v-model="choice.ChoiceName" />
                                                    </div>
                                                    <div class="form-control w-32">
                                                        <div class="relative">
                                                            <input type="number" class="input input-sm input-bordered w-full pr-8 text-right focus:input-primary bg-slate-50 border-slate-200 h-10" v-model="choice.price" @keypress="onlyNumber" />
                                                            <span class="absolute right-3 top-2.5 text-slate-400 text-sm">฿</span>
                                                        </div>
                                                    </div>
                                                    <button @click="removeChoice(gIndex, cIndex)" class="btn btn-square btn-ghost btn-sm h-10 w-10 text-slate-400 hover:text-red-500 hover:bg-red-50">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                                            ...
                                            <button @click="addChoice(gIndex)" class="btn btn-sm btn-outline border-dashed border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-300 w-full md:w-auto font-medium font-sans">
                                ...
                                <div v-if="!MenuData.OptionGroups || MenuData.OptionGroups.length === 0" class="py-12 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50/50">
                                    <div class="text-slate-300 mb-4">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-10 h-10 text-slate-300">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 13.5V10.5m0 0V7.5m0 3h3m-3 0h-3m12 3V10.5m0 0V7.5m0 3h3m-3 0h-3M6 19.5v-3m0 3h3m-3 0h-3m12 3v-3m0 3h3m-3 0h-3M12 13.5V10.5m0 0V7.5m0 3h3m-3 0h-3m0 9v-3m0 3h3m-3 0h-3" />
                                        </svg>
                                    </div>
