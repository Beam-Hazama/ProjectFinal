...
const menus = [
  {
    name: 'Order List',
    routeName: 'Restaurants Orderlist',
  ...
  {
    name: 'Order History',
    routeName: 'Restaurants Orderhistory',
  ...
  {
    name: 'Menu List',
    routeName: 'Restaurants Menulist',
...
onMounted(async () => {
  ...
  // โหลดข้อมูลร้าน (ครั้งแรกเท่านั้น)
  if (!profileStore.docId) {
    await profileStore.fetchRestaurantByName();
  }

  // ตรวจสอบและอัปเดตสถานะทันทีเมื่อเปิดระบบ
  await profileStore.updateStatusByTime();

  // ตรวจสอบสถานะทุก 1 นาที
            ...
            <aside class="w-72 min-h-full bg-white shadow-xl flex flex-col border-r border-slate-100">
                <div class="h-20 flex items-center px-6 border-b border-slate-100 shrink-0">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl font-extrabold text-slate-800 tracking-tight">Restaurant<span class="text-blue-600">.</span></h1>
                    ...
                    <div class="flex items-center gap-3 mb-4 px-2">
                        <div class="avatar online">
                            <div class="w-10 rounded-full ring ring-blue-600 ring-offset-base-100 ring-offset-2">
                                <img :src="accountStore.user?.ImageUrl || 'https://api.dicebear.com/7.x/avataaars/svg?seed=RestaurantUser'" />
                            </div>
                        </div>
