    ...
    <button v-for="(menu, index) in sortedMenus" :key="menu.id"
      ...
      <template v-if="layout === 'horizontal'">
        <figure class="w-[100px] h-full flex-shrink-0 relative bg-gray-100 flex items-center justify-center border-r border-gray-50">
          <img v-if="menu.ImageUrl" :src="menu.ImageUrl" class="object-cover w-full h-full" :class="{ 'grayscale': menu.Status !== 'open' || isShopClosed(menu.Restaurant) }" />
          ...
          <div v-if="isShopClosed(menu.Restaurant)" class="absolute inset-0 bg-black/40 flex items-center justify-center">
      ...
      <template v-else>
        <div class="w-full aspect-square relative bg-gray-100 overflow-hidden">
          ...
          <div v-if="isShopClosed(menu.Restaurant)" class="absolute inset-0 bg-black/40 flex items-center justify-center z-10">
            <span class="text-white font-bold text-[10px] bg-gray-800/80 px-2 py-1 rounded-md shadow-sm">ร้านปิด</span>
          </div>
