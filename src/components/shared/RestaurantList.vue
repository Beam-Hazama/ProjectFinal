...
const isShopClosed = (shop) => restaurantStore.isShopClosedByName(shop.Restaurant, now.value);
...
<template>
    <section class="flex flex-col gap-3 pb-4 px-2">
        <button v-for="shop in sortedRestaurants" :key="shop.id"
            ...
            @click="!isShopClosed(shop) && goToRestaurantMenu(shop.Restaurant)">
            ...
            <div class="py-2 px-3 w-full flex flex-col justify-center flex-grow bg-white min-w-0">
                <h3 class="font-bold text-[15px] text-gray-800 leading-tight truncate w-full mb-0.5">{{ shop.Restaurant }} </h3>
                <p v-if="getRestaurantCategories(shop.Restaurant)" class="text-[11px] text-gray-500 font-medium truncate mb-1 opacity-80">
                    {{ getRestaurantCategories(shop.Restaurant) }}
                </p>
