...
<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex flex-col gap-3 py-4 px-4 text-gray-700">
    ...
    <div class="flex justify-between items-center  md:px-0 mb-2">
      ...
      <router-link to="/user" class="group flex items-center gap-2 bg-white/80 backdrop-blur-sm px-3 py-2 rounded-xl shadow-sm text-blue-600 hover:bg-blue-600 hover:text-white hover:shadow-md transition-all duration-300 border border-white/50">
    ...
    <div v-for="order in Order.list" :key="order.id" class="bg-white w-full rounded-3xl shadow-xl relative flex flex-col h-fit overflow-hidden ">
      <div class="bg-white p-4 pb-2">
        <div class="bg-blue-50 rounded-2xl p-6 border border-blue-100 flex justify-between items-center">
          <div>
            ...
            <div class="text-3xl font-bold text-blue-800 mt-1">#{{ order.OrderNumber || 'N/A' }}</div>
          </div>
          <div class="text-right">
            <div class="text-xs text-blue-400 font-bold uppercase tracking-wider mb-1 mx-2.5">Status</div>
            <div class="inline-block bg-white text-blue-600 text-sm font-bold px-4 py-1.5 rounded-lg shadow-sm mb-2">
      ...
      <div class="px-3 py-2">
        <div v-for="(item, index) in order.Menu" :key="index" class="flex items-center py-4 border-b border-gray-100 last:border-0 group hover:bg-gray-50 transition-colors rounded-xl px-2 -mx-2">
      ...
      <div class="bg-white p-7 pb-10 space-y-4 z-10 ">
        ...
        <div class="pt-2 text-xs text-gray-400 space-y-2 text-right">
          <div class="flex justify-between">
            <span>VAT 7%</span>
            <span>{{ calculateVat(calculateGrandTotal(calculateSubtotal(order.Menu))).toFixed(2) }} บาท</span>
          </div>
          <div class="flex justify-between">
            <span>ราคาไม่รวมภาษี (Excl. VAT)</span>
            <span>{{ calculateExclVat(calculateGrandTotal(calculateSubtotal(order.Menu))).toFixed(2) }} บาท</span>
          </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center py-2 space-y-2">
          ...
          <div class="p-3 bg-white rounded-2xl shadow-md border border-gray-100">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=Example" alt="QR Code" class="w-28 h-28 mix-blend-multiply opacity-90" />
          </div>
        </div>
      </div>
    </div>
