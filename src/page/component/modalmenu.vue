  ...
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    ...
    <div class="bg-white rounded-xl w-80 shadow-lg overflow-hidden">
      ...
      <div class="p-4 space-y-3">
        ...
        <textarea v-model="note" class="textarea textarea-bordered w-full" placeholder="หมายเหตุ" />
      </div>


      <div class="flex border-t">
        <button class="w-1/2 py-3 text-gray-500 hover:bg-gray-100" @click="emit('close')">
          ยกเลิก
        </button>
        <button class="w-1/2 py-3 bg-green-500 text-white hover:bg-green-600" @click="confirmAdd">
          ยืนยัน
        </button>
      </div>
