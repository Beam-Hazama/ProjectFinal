  ...
  <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
    ...
    <div class="flex lg:flex-row flex-col gap-2 w-full lg:w-auto mt-4 md:mt-0 items-center">
      ...
      <div class="flex flex-col lg:flex-row gap-2 w-full lg:w-auto">
        ...
        <div class="dropdown w-full lg:w-auto">
          <label tabindex="0" class="btn btn-sm bg-white border-slate-200 text-slate-600 font-normal hover:bg-slate-50 w-full justify-between lg:min-w-[160px] h-[32px] min-h-[32px] px-3">
            <span v-if="dashboardStore.menuCategoryFilters.length === 0">ทุกหมวดหมู่อาหาร</span>
            <span v-else class="text-indigo-600">เลือกแล้ว {{ dashboardStore.menuCategoryFilters.length }} หมวดหมู่</span>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-2 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 9l-7 7-7-7" /></svg>
          </label>
          <ul tabindex="0" class="dropdown-content z-[10] menu p-2 shadow-xl border border-slate-100 bg-base-100 rounded-box w-72 max-h-[60vh] overflow-y-auto mt-2 flex-nowrap">
            <li class="sticky top-0 bg-base-100 z-10 border-b border-slate-100 pb-1 mb-1">
              <a @click="dashboardStore.clearCategoryFilters()" class="text-rose-500 font-medium justify-center py-2" :class="{ 'opacity-50 pointer-events-none': dashboardStore.menuCategoryFilters.length === 0 }">ล้างการเลือกทั้งหมด</a>
            </li>
            <li v-for="category in dashboardStore.availableCategories" :key="category">
              <label class="label cursor-pointer flex justify-start gap-3 py-2">
                <input type="checkbox" :checked="dashboardStore.menuCategoryFilters.includes(category)" @change="dashboardStore.toggleCategoryFilter(category)" class="checkbox checkbox-sm checkbox-primary rounded-full border-indigo-600" />
                <span class="label-text whitespace-normal font-medium leading-tight text-slate-600">{{ category }}</span>
              </label>
            </li>
          </ul>
        </div>
      ...
      <div class="dropdown dropdown-end w-full lg:w-auto">
        <label tabindex="0" class="btn btn-sm bg-white border-slate-200 text-slate-600 font-normal hover:bg-slate-50 w-full justify-between lg:min-w-[160px] h-[32px] min-h-[32px] px-3">
          <span v-if="dashboardStore.menuFilters.length === 0">ทุกเมนูอาหาร</span>
          <span v-else class="text-indigo-600">เลือกแล้ว {{ dashboardStore.menuFilters.length }} เมนู</span>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-2 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 9l-7 7-7-7" /></svg>
        </label>
        <ul tabindex="0" class="dropdown-content z-[10] menu p-2 shadow-xl border border-slate-100 bg-base-100 rounded-box w-72 max-h-[60vh] overflow-y-auto mt-2 flex-nowrap">
          <li class="sticky top-0 bg-base-100 z-10 border-b border-slate-100 pb-1 mb-1">
            <a @click="dashboardStore.clearMenuFilters()" class="text-rose-500 font-medium justify-center py-2" :class="{ 'opacity-50 pointer-events-none': dashboardStore.menuFilters.length === 0 }">ล้างการเลือกทั้งหมด</a>
          </li>
          <li v-for="menu in dashboardStore.availableMenus" :key="menu.id">
            <label class="label cursor-pointer flex justify-start gap-3 py-2">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-[10px] text-slate-400 mt-1" v-if="menu.Restaurant">{{ menu.Restaurant }}</span>
              </div>
            </label>
          </li>
          <li v-if="dashboardStore.availableMenus.length === 0" class="p-4 text-center text-slate-400 text-sm pointer-events-none">ไม่พบเมนู</li>
        </ul>
      </div>

      <button v-if="dashboardStore.hasActiveFilters" @click="dashboardStore.clearAllFilters()" class="btn btn-sm bg-rose-50 text-rose-500 border-none font-medium hover:bg-rose-100 h-[32px] min-h-[32px] px-4 shadow-sm ml-1">ล้างทั้งหมด</button>
    </div>
  </div>
</template>
