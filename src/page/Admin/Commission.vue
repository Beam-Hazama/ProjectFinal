        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all duration-300">
          ...
          <div class="relative z-10 flex justify-between items-start">
            <div>
              ...
              <h3 class="text-3xl font-black text-slate-800">฿{{ formatPrice(commissionStore.totalSystemRevenue) }}</h3>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all duration-300">
          ...
          <div class="relative z-10 flex justify-between items-start">
            <div>
              ...
              <h3 class="text-3xl font-black text-rose-600">฿{{ formatPrice(commissionStore.totalCommissions) }}</h3>
              <p class="text-xs font-bold text-rose-400 mt-4 flex items-center gap-1">
                ...
                Avg Rate: {{ ((commissionStore.totalCommissions / (commissionStore.totalSystemRevenue || 1)) * 100).toFixed(1) }}%
              </p>
            </div>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-all duration-300">
          ...
          <div class="relative z-10 flex justify-between items-start">
            <div>
              ...
              <h3 class="text-3xl font-black text-emerald-600">฿{{ formatPrice(commissionStore.totalNetPayout) }}</h3>
        ...
        <div class="overflow-x-auto">
          <table class="table w-full">
            ...
            <tbody class="text-slate-600">
              <tr v-for="shop in commissionStore.restaurantData" :key="shop.name"
                ...
                <td class="text-center">
                  <div class="flex items-center justify-center gap-2">
                    <input type="number" min="0" max="100" :disabled="!commissionStore.isEditing" :value="shop.rate"
                      @input="commissionStore.updateLocalRate(shop.name, $event.target.value)" :class="[
                        'input input-sm input-bordered w-20 text-center font-bold',
                        commissionStore.isEditing ? 'text-indigo-600 border-indigo-200' : 'text-slate-400 bg-slate-50 border-transparent'
                      ]" />
                  </div>
                </td>
            ...
            <tfoot class="bg-slate-50/50">
              <tr>
                ...
                <th class="text-right">฿{{ formatPrice(commissionStore.totalSystemRevenue) }}</th>
                <th></th>
                <th class="text-right">฿{{ formatPrice(commissionStore.totalCommissions) }}</th>
                <th class="text-right">฿{{ formatPrice(commissionStore.totalNetPayout) }}</th>
              </tr>
            </tfoot>
          </table>
        </div>
